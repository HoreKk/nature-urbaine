...
A pnpm + Turbo monorepo with two apps and one shared package. PostgreSQL is the single backing store.
...
- **Shared (`packages/database`)** — exports the Payload `config`, generated TypeScript types (`Report`, `Interview`, etc.), Lexical helpers, and migration / seed scripts. Both apps import from `@nature-urbaine/database`.
...
| Contribution                             | (not modeled yet)                        | The "Contribuer" form on the website does not yet persist to a `Submission` collection.                                                                                                                          |
...
| Contributions              | `/contribuer`                  | `routes/contribuer.tsx`                  | Spec form: project name, location, date, description (≤500 chars), category, MOA, MOE, surface, cost HT, photos, photo author, contact email. **Submission is not currently persisted** (no `Submission` collection). |
| Contact                    | `/contact`                     | `routes/contact.tsx`                     | Spec form: prénom, nom, e-mail, secteur d'activité, message.                                                                                                                                                          |
...
| `reports.ts`             | `getReports`, `getReportById`                            | Paginated list with filters (`category`, `city`, free-text); detail with relations resolved. Augments raw `Report` into `AugmentedReport` (thumbnail, category, pictures fully fetched). |
...
- Should `Submission` (Contribuer form persistence) live as a Payload collection with workflow states (`pending` / `accepted` / `rejected`), or be sent by email only?
